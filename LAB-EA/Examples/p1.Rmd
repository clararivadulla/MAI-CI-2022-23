---
title: "Evolutionary Computation Laboratory Part 1: **Genetic Algorithms** (the first part of this file is largely based on _A quick tour of GA_ by Luca Scrucca)"
...
This is because GAs are good "scouters" (global searchers) but, once a promising solution area is reached, convergence to the local optimum can take time; local search algorithms (like those based on derivatives) have the opposite behaviour. Their combination is thus a very good idea in general^[In evolutionary biology, this is sometimes known as _Lamarckian evolution_.], although the computational cost can be prohibitive, and it should be done with this issue in mind.
...
The Knapsack Problem is one of the famous problems in optimization; one can take it as a paradigmatic example, and many problems can be cast in this form.
...
The _decision problem_ form of the knapsack problem (can a value of at least V be achieved without exceeding the weight W?) is NP-complete, thus there is no known algorithm that is both correct and fast (polynomial-time) in all cases. You can find detailed information in https://en.wikipedia.org/wiki/Knapsack_problem#0-1_knapsack_problem
...
# generate random feasible weights (%/% is integer division)
