...
author: "Benjamí Parellada and Clara Rivadulla"
...
Function to round numeric values of a dataframe:
```{r}
round_df <- function(df, digits, max_value) {
...
Function to generate a dataframe of N data instances and split it into train, validation and test sets according to the fractions given (`fractionTraining`, `fractionValidation`, and `fractionTest`):
...
generate_data <- function(N, fractionTraining, fractionValidation, fractionTest) {
  ...
  correlated_vars_df <- round_df(correlated_vars_df, digits = 1, max_value = 10)
  
  df <- cbind(absences, correlated_vars_df)
...
We generate 3 different data frames: one as the `train` set, one as the `validation` set and one as the `test` set. 
...
```{r}
library(nnet)
model <- nnet(prog_proj_grade/10 ~ ., data=train, size=5, decay=1.0e-5, maxit=2000)
model
summary(model)
```
```{r}
...
fit_rf=function(x)
{
  ...
  model=nnet(prog_proj_grade/10 ~ ., data=train, size=s)
...
ga_size=apply(ga_rf_fit[, 1:5],1,binary2decimal)
ga_size
```
